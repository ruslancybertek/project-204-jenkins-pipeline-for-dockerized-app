pipeline{
    ...
    environment {
        ...
        CFN_KEYPAIR = "linux.pem"
        HOME_FOLDER = "/home/ec2-user"
        GIT_FOLDER = sh(script:'echo ${GIT_URL} | sed "s/.*\\///;s/.git$//"', returnStdout:true).trim()
        PATH=sh(script:"echo $PATH:/usr/local/bin", returnStdout:true).trim()
        app_name = "phonebook-app"
    }
    stages {
        ...
        stage('Test the Infrastructure'){
            steps {
                echo 'Testing if Docker Swarm is ready or not by checking the Viz App on Grand Master with public IP:
                ${MASTER_INSTANCE_PUBLIC_IP}:8080'
        ...
        stage('Deploy the App on Docker Swarm'){
            environment{
                MASTER_INSTANCE_ID = sh(script:'aws ec2 describe-instances --region ${AWS_REGION} --filters Name=tag-value,Values=docker-grand-master Name=tag-value,
                        Values=${AWS_STACK_NAME} --query Reservations[*].Instances[*].[InstanceId] --output text | sed "s/\\s*None\\s*//g"', returnStdout:true).trim()')
            }
            steps {
    ...
    post {
        ...
        failure {
            ...
            sh 'aws clouformation delete-stack --region ${AWS_REGION} --stack-name ${AWS_STACK_NAME}'
        }
    }
}